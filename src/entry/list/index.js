  ...
  try {
    ...
    if (user.role_id.name.toLowerCase() == "administrator") {
      ...
      if (userWarehouseIds.length > 0) {
        ...
        entries = await Entry.find({ user_id: user.id, warehouse_id: { $in: ids } }).populate("warehouse_id", { name: 1, location_id: 1 }).populate("product_id", { name: 1 }).populate("user_id").sort({ createdAt: 'desc' });
      }
    }
